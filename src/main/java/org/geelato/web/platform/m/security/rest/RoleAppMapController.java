...
public class RoleAppMapController extends BaseController {
    ...
    public ApiPagedResult pageQuery(HttpServletRequest req) {
        ...
        try {
            ...
            Map<String, Object> params = this.getQueryParameters(RoleAppMap.class, req);
            FilterGroup filterGroup = this.getFilterGroup(params, OPERATORMAP);

            List<RoleAppMap> pageQueryList = roleAppMapService.pageQueryModel(RoleAppMap.class, pageNum, pageSize, filterGroup);
            List<RoleAppMap> queryList = roleAppMapService.queryModel(RoleAppMap.class, filterGroup);
    ...
    public ApiResult query(HttpServletRequest req) {
        ...
        try {
            Map<String, Object> params = this.getQueryParameters(RoleAppMap.class, req);
            return result.setData(roleAppMapService.queryModel(RoleAppMap.class, params));
        } catch (Exception e) {
            logger.error(e.getMessage());
            result.error().setMsg(ApiErrorMsg.QUERY_FAIL);
        }
    ...
    public ApiResult insert(@RequestBody RoleAppMap form) {
        ApiResult result = new ApiResult();
        try {
            roleAppMapService.insertModel(form);
        } catch (Exception e) {
            logger.error(e.getMessage());
    ...
    public ApiResult isDelete(@PathVariable(required = true) String id) {
        ...
        try {
            ...
            if (rResult != null) {
                roleAppMapService.isDeleteModel(rResult);
                result.success();
            } else {
